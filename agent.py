...
def create_article_image(title, keyword, slug):
    os.makedirs(IMAGE_DIR, exist_ok=True)
    path = os.path.join(IMAGE_DIR, f"{slug}.png")
    width, height = 1600, 900
    seed = sum(ord(c) for c in (title + keyword))
    palettes = [((8,25,20),(46,96,70)),((18,28,45),(49,86,115)),((35,24,18),(112,78,48)),((22,36,28),(78,111,66))]
    c1,c2=palettes[seed % len(palettes)]
    img=Image.new("RGB",(width,height),c1); d=ImageDraw.Draw(img)
    for x in range(width):
        t=x/(width-1); col=tuple(int(c1[i]*(1-t)+c2[i]*t) for i in range(3)); d.line([(x,0),(x,height)],fill=col)
    for i in range(7):
        r=80+((seed+i*97)%220); x=(seed*(i+3)*17)%width; y=(seed*(i+5)*11)%height
        d.ellipse((x-r,y-r,x+r,y+r),outline=(210,235,190),width=3)
    ...
    tf=font(font_paths,72); sf=font(regular_paths,28); bf=font(font_paths,34)
    d.rounded_rectangle((90,90,1510,810),radius=42,fill=(5,14,12),outline=(190,230,150),width=3)
    d.text((145,140),"STRONGERYEARS",font=bf,fill=(190,240,110))
    lines=[]; line=""
    for word in title.split():
        test=(line+" "+word).strip(); box=d.textbbox((0,0),test,font=tf)
        if box[2]<=1250: line=test
        else:
            if line: lines.append(line)
            line=word
    if line: lines.append(line)
    y=245
    for line in lines[:4]: d.text((145,y),line,font=tf,fill=(248,248,242)); y+=86
    d.text((145,735),"Healthy aging  •  Strength  •  Mobility  •  Vitality",font=sf,fill=(210,220,210))
    img.save(path,"PNG",optimize=True)
    return f"{SITE_URL}/content/images/{slug}.png"

def article_html(title, description, keyword, body_html, related, published_at=None, image_url=None):
    safe_title = html.escape(title)
    image_url = image_url or f"{SITE_URL}/content/images/{slugify(title)}.png"
...
def build_homepage(articles):
    ...
    for i, (title, name) in enumerate(articles[:8]):
        slug = Path(name).stem
        img = f"{SITE_URL}/content/images/{slug}.png"
...
def main():
    ...
    for offset in range(len(TOPICS)):
        ...
        try:
            ...
            image_url = create_article_image(title, keyword, slug)
            page = article_html(
                title,
                desc,
                keyword,
